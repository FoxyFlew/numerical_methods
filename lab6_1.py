...
def equation(f, l, T, m1, m2, s1, s2, I, h, tau):
    solution = []
    nx = int(l / h) + 1
    nt = int(T / tau) + 1
...
num = equation(op_exact, 1, 0.1, m1, m2, sigma1, sigma2, lambda x: exact_solution(x, 0), 0.1, 0.005)
exa = fill_exact_matrix(1, 0.1, 0.1, 0.005)
print("h = 0.1, tau = 0.005, error:", calculate_error(num, exa))

num = equation(op_exact, 1, 0.1, m1, m2, sigma1, sigma2, lambda x: exact_solution(x, 0), 0.1, 0.0025)
exa = fill_exact_matrix(1, 0.1, 0.1, 0.0025)
print("h = 0.1, tau = 0.0025, error:", calculate_error(num, exa))

num = equation(op_exact, 1, 0.1, m1, m2, sigma1, sigma2, lambda x: exact_solution(x, 0), 0.05, 0.00125)
...
num = equation(op_exact, 1, 0.1, m1, m2, sigma1, sigma2, lambda x: exact_solution(x, 0), 0.2, 0.005)
exa = fill_exact_matrix(1, 0.1, 0.2, 0.005)
print("h = 0.2, tau = 0.005, error:", calculate_error(num, exa))

num = equation(op_exact, 1.0, 0.1, m1, m2, sigma1, sigma2, lambda x: exact_solution(x, 0), 0.2, 0.0025)
